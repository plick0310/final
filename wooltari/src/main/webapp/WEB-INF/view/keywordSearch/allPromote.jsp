...
<div style="margin: auto; width: 900px;">
	<div style="padding-top: 50px; font-size: 17px; ">
		<table class="tab_menu" style="width:100%; float: left; display: block; text-align: left;  border-bottom: 1px solid #e2e2e2; margin-bottom: 50px; ">
			<tr>
				...
				<td style="height:50px; line-height:50px; margin-left:30px; float: left; position: relative; list-style-type: none; font-family: 'BareunDotum',gulim,helvetica,sans-serif; border-bottom:2px solid #1abc9c;"><a style="cursor: pointer; color: #1abc9c;" onclick="location.href='<%=cp%>/keyword/promoteSearch?keyword=${keyword }'">홍보</a></td>
	...
	<div style="clear: both; font-size: 16px; font-family:'BareunDotum'; font-weight: bolder; color: black; margin-bottom: 20px; margin-top: 79px;">홍보게시판[검색결과:${dataCount }개]</div>
		...
		<ul>
						...
						<c:if test="${dto.imageFileName != null }">
							...
							<dl style="padding-top: 3px; overflow: hidden; line-height: 19px; display: block;">
								<dt style="display: inline; clear: both; margin-right: 5px; "><a onclick="location.href='<%=cp%>/keyword/promote/article?num=${dto.num }'" style="cursor: pointer; color: #1abc9c; font-size: 15px;">${dto.subject }</a>&nbsp;&nbsp;</dt>
								<dd class="txt_inline" style="clear: both; margin: 2px 0 1px; font-size: 12px; line-height: 18px; display: inline;">작성일|${dto.created }</dd>
							<dt style="font-size:11px; margin-top: 8px;">[스터디명:${dto.studyName }][조회수&nbsp;&nbsp;${dto.hitCount }]</dt>								
