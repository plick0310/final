...
$(function(){
    ...
    var uig = "${sessionScope.member.userImg}";
    if(! uid) {
	...
	// ---------------------------------------------
	// 채팅방 제목을 클릭한 경우
	$("body").on("click", "#roomListContainer tr", function(){
		$("#roomListContainer tr").each(function(){
	...
	$("#btnRoomJoin").click(function(){
		if(! $("#roomListContainer tr").hasClass("selection")) {
			alert("입장할 채팅방을 먼저 선택 하세요 !!!");
			return;
		}
		
		var $div = $("#roomListContainer .selection");
		var roomId = $div.attr("data-roomId");
	...
	function roomProcerss(data) {
		...
		if(cmd=="room-list") {
			...
			$("#roomListContainer").append("<table class='table-striped'><tr class='row' id='row1' data-roomId='"+roomId+"'><td><img src='<%=cp%>/uploads/member/userImg/"+roomImg+"' class='img-circle' width='35px' height='35px' style='padding:1px; box-shadow: 2px 2px 5px grey;'></td><td>"+subject+"</td></tr></table>");
			...
			if($("#roomListContainer tr[data-roomId='"+roomId+"']").hasClass("selection")) {
				$("#roomInfoContainer").empty();
			}
			
			$("#roomListContainer tr[data-roomId='"+roomId+"']").remove();
		}
	...
	function talkProcerss(data) {
		...
		if(cmd=="join-list") {
			...
			$("#chatRoomJoinList div[data-guestId="+guestId+"]").remove();
		}
	...
	$("body").on("click", "#chatRoomJoinList div", function(){
...
<div class="body-container" style="width: 700px;">
    <div class="body-title">
		...
		style="color: rgba(183, 183, 183, 0.65); font-size: 20px;"> <i
			...
			style="color: #1abc9c; margin-bottom: 20px;"></i>&nbsp;&nbsp;채 팅
	</span> C H A T T I N G</strong>
    </div>
    ...
    <div style="clear: both;">
        <div style="float: left; width: 300px;">
            ...
            <div style="clear: both; padding-top: 5px;">
                <button type="button" id="btnRoomAdd" class="clickbtn">채팅방 만들기</button>
                <button type="button" id="btnRoomJoin" class="clickbtn">채팅방 들어가기</button>
            </div>
        </div>
