...
function deleteBoard() {
	  ...
	  var url = "<%=cp%>/community/usedshop/delete?" + query;
...
<table cellpadding="0" cellspacing="0" id="read_table" class="board_table" style="width: 900px; margin: -45px auto 0;">
	...
	<tbody>
		...
		<div style="background: #eee; width: 100%; padding: 10px;line-height: 25px;">
				<div class="row" >
				 <div class="col-md-4"><span class="infobo"><i class="glyphicon glyphicon-flag"></i>지역 :</span> </div>
				  <div class="col-md-4"><span class="infobo"><i class="glyphicon glyphicon-time"></i>영업시간 :</span> </div>
				   <div class="col-md-4"><span class="infobo"><i class="glyphicon glyphicon-phone-alt"></i> 전화번호 :</span> </div>
				  
					</div>
					<div class="row">
				 <div class="col-md-12"><span class="infobo"><i class="glyphicon glyphicon-map-marker"></i>주소 :</span></div>
					</div>
		</div>
			<div>
				<input class="clickbtn" type="button" value="구매하기" onclick="javascript:location.href='<%=cp%>/community/usedshop/purchase';">
					...
					<tr style=" border-top: 1px solid #eee;">
						...
						<c:if test="${not empty preReadDto }">
			       		<a href="<%=cp%>/community/usedshop/article?${query }&num=${preReadDto.num}">${preReadDto.subject }</a>
			       </c:if>
						</strong></td>
					</tr>
					...
					<c:if test="${not empty nextReadDto }">
						<a href="<%=cp%>/community/usedshop/article?${query }&num=${nextReadDto.num}">${nextReadDto.subject }</a>
					</c:if>
						</strong></td>
					</tr>
			...
			<tr height="45" style="    border-top: 1px solid #eee;">
			    <td width="300" align="left">
			    	<c:if test="${sessionScope.member.userId==dto.userId }">
			          <button type="button" class="clickbtn" onclick="javascript:location.href='<%=cp%>/community/usedshop/update?num=${dto.num}&page=${page}';" 
			    ...
			    <td align="right">
			        <button type="button" class="clickbtn" onclick="javascript:location.href='<%=cp%>/community/usedshop/list?${query }';">리스트</button>
			    </td>
			</tr>
