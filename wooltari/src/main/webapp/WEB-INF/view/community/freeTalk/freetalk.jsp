...
function listPage(page) {
	...
	$.ajax({
		...
		success:function(data) {
			printFreeTalk(data); // 아래 printGuest로
		}
		,error:function(e) {
...
//	out+=" 		  <span style='color: #3EA9CD; font-weight: 700;'>Today:  "+todayCount+"</span>";
	...
	if(dataCount!=0) {
		for(var idx=0; idx<data.list.length; idx++) {
			...
			if(uid==userId || uid=="admin") {
				out+=" | <a onclick='deleteFreeTalk(\""+num+"\", \""+pageNo+"\");'>삭제</a></td>" ;
			} else {
				out+=" | <a href='#'>신고</a></td>" ;
			}
			out+="    </tr>";
	...
	$("#listFreeTalk").html(out);
}

function sendFreeTalk() {
	var url="<%=cp%>/freeTalk/insert";
	var query=$("form[name=freeTalkForm]").serialize();
...
function deleteFreeTalk(num, page) {
	var uid="${sessionScope.member.userId}";
	if(! uid) {
		alert("로그인이 필요합니다");
		return;
...
<div class="body-container" style="width: 700px;">
    ...
    <div>
             <form name="freeTalkForm" method="post" action="">
             <div class="freeTalk-write">
                 <div style="clear: both; padding-top: 10px;">
                  ...
                  <div style="text-align: right; padding-top: 10px;">
                       <button type="button" class="btn" onclick="sendFreeTalk();" style="padding:8px 25px;"> 등록하기 </button>
                  </div>           
            </div>
           </form>
